/*
  This website explain the sha1 algorithm
...
void leftRotate(str& strBin,uint bits){
  /* it will rotate leftmost bits from strBin toward its rightmost */
...
str _not(const str& strBin){
  /* it will generate the opposite strBin */
  str negStrBin = "";
...
str strToBin(str _str){
  /* it will turn a string into 8 bits binary group for each letter */
  str binary = "";
...
void fillWithZeroes(str& strBin, uint zeroes){
  /* it will fill strBin with zeroes */
...
void processChunk(uint start,str& strBin,str& h0,str& h1,str& h2,str& h3, \
  ...
  for(uint i=16; i<wordsAmount; i++){
    ...
    // carrying newWord leftmost bit to its rightmost bit
...
str turnGroupToHex(str& hi,uint start){
  /* it will build a 4 bits group and turn it to decimal number,
    then it will turn it to hexadecimal number */
