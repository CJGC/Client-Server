...
class tracker {
 ...
 private:
  _map keys;
  str id, ip, port, aftId, aftIp, aftPort, befId, befIp, befPort, auxId,
      publIpAux, publPortAux, amILast;
 ...
 private:
  ...
  void printerFunct(socket& printer, vec& info) {
    ...
    message request, answer;
    for (auto item : info) request << item;
 ...
 public:
  ...
  void client(socket& cli, socket& printer, socket& publ) {
    ...
    publ.bind("tcp://" + this->publIp + ":" +
              this->publPort);  // binding publisher
    if (this->publIp == "*") this->publIp = "localhost";
    str userOp;
    cout << "type something to bind with ring chord and upgrade this finger "
            "table: ";
    getline(cin, userOp);
    bindWithChord(cli);
    this->canIStart = true;
    ...
    cout << "type something to send unsubscribe request to my subscribers and "
            "exit: ";
    getline(cin, userOp);
    publisher(publ);
    if (!this->_exit) unbindWithChord(cli);
    /* ----- Sebastian's printer section ----- */
    info.clear();
    info.resize(6);
    info[0] = "delete node";
 ...
 private:
  void unbindWithChord(socket& cli) {
    ...
    if (this->id < this->befId)  // first node with last node case
      this->amILast = "true";
    request << "setinfo" << this->aftId << this->aftIp << this->aftPort
            << this->amILast << keys;
    cli.send(request);
    cli.receive(answer);
  ...
  void bindWithChord(socket& cli) {
    ...
    while (true) {
      ...
      cli.disconnect("tcp://" + this->aftIp + ":" + this->aftPort);
      this->aftIp = remtNextIp;
      this->aftPort = remtNextPort;
    }
  }
  ...
  void reqSetInfo(socket& cli, str& remtIsLast, str& remtId) {
    ...
    request << "setinfo" << this->id << this->ip << this->port << remtIsLast
            << " ";
    cli.send(request);
    cli.receive(answer);
    setBefInfo(remtId, this->aftIp, this->aftPort);
  ...
  void server(socket& serv, socket& cli, socket& subs, socket& printer) {
    ...
    while (!this->_exit) {
      ...
      if (op == "getinfo")
        ...
        upgrFinTabl(subs, printer, cId, cIp, cPort);
        reply << "ok";
      }
 ...
 private:
  ...
  void getInfo(message& package) {
    ...
    package << this->id << this->aftId << this->aftIp << this->aftPort
            << this->amILast;
  }
 ...
 public:
  void subscriber(socket& subs, socket& aux, socket& printer) {
    ...
    if (this->amILast != "true") setFinTabl(aux);
    conWithAllPubl(subs);
    conWithMyPubl(subs);
    listenPubl(subs, aux, printer);
    disconWithMyPubl(subs);
  }

 private:
  void listenPubl(socket& subs, socket& aux, socket& printer) {
    ...
    while (true) {
      ...
      if (part1 == "external disconnect" && part2 != this->id)
        upgrFinTabl(subs, aux, printer, part2, part3, part4, part5);
      else if (part1 == "disconnect" && part2 == this->id)
        break;
    }
  }

  void upgrFinTabl(socket& subs, socket& aux, socket& printer, str& id, str& ip,
                   str& port, str& port2) {
    ...
    bool thereWasUpgrade = false;  // upgrade state
  ...
  void upgrFinTabl(socket& subs, socket& printer, str& id, str& ip, str& port) {
    ...
    */
    ...
    if (id > this->id) {
      ...
      for (auto& item : this->finTabl) {
        str key = item.first;
        if (key <= id) {
          str oldIpAux = item.second[0], oldPortAux = item.second[1],
  ...
  void searchIntoRing(socket& aux, _map::iterator& item) {
    ...
    while (true) {
      ...
      if (key <= remtId || remtIsLast == "true") {
        if (remtIsLast == "true" && key > remtId) break;
        item->second[0] = remtPublIp;
        item->second[1] = remtPublPort;
        item->second[2] = remtId;
        break;
  ...
  void setFinTabl(socket& aux) {
    ...
    for (auto& it = start; it != end; it++) searchIntoRing(aux, it);
  }

  void getIdsFinTabl(str& idsFinTabl) {
    /* it will get ids logged into finger table */
    for (auto& item : this->finTabl) idsFinTabl += item.second[2] + " ";
  }
...
int main(int argc, const char** argv) {
  ...
  tracker track(id, localIp, "5555", localIp, "5556", aftIp, "5557", ownFiles);

  thread t0(&tracker::UpgrFinTablnotify, &track, ref(cli)),
         t1(&tracker::server, &track, ref(serv), ref(cli), ref(subs),ref(printer)),
         t2(&tracker::subscriber, &track, ref(subs), ref(aux), ref(printer));
